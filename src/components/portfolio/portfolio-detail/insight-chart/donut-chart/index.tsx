...
import chroma from 'chroma-js';
  ...
  export const DonutChart = ({ pieChartData }: IProps) => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
    
    const totalValue = pieChartData.reduce((total,cur,idx)=>cur.sumValue+total,0);
    const series = pieChartData.map((item) => Math.round(item.sumValue*10000/totalValue)/100);
