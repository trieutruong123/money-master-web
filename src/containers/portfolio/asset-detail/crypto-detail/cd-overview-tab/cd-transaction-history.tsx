...
const CDTransactionHistory = ({ transactionHistoryData, content }: IProps) => {
  ...
  const resetTransaction = async () => {
    await cryptoDetailStore.resetTransactionSelection();
    resetPageNumbers();
  };
  ...
  const resetPageNumbers = () => {
    ...
    const count =
      Math.floor(total / TransactionHistoryContants.itemsPerPage) +
      (total % TransactionHistoryContants.itemsPerPage ? 1 : 0);
  ...
  const handlePageChange = async (pageNumber: number) => {
    const transactionHistory =
      (cryptoDetailStore.transactionHistory &&
        cryptoDetailStore.transactionHistory.slice()) ||
      [];
    ...
    if (pageNumber == count) {
      const startDate = cryptoDetailStore.transactionSelection.startDate
        ? dayjs(cryptoDetailStore.transactionSelection.startDate)
            .startOf('day')
            .format()
        : null;
      const endDate = cryptoDetailStore.transactionSelection.endDate
        ? dayjs(cryptoDetailStore.transactionSelection.endDate)
            .endOf('day')
            .format()
        : null;
        ...
        >
          ...
          <Box>
            <Table>
              ...
              <TableBody>
                {transactionHistoryData
                  .slice(
                    (cryptoDetailStore.currentPage - 1) *
                      TransactionHistoryContants.itemsPerPage,
                    cryptoDetailStore.currentPage *
                      TransactionHistoryContants.itemsPerPage,
                  )
                  .map((record, i) => {
                    return (
            ...
            <Pagination
              ...
              currentPage={cryptoDetailStore.currentPage}
