...
const CDTransactionHistory = observer(
  ({ transactionHistoryData, content }: IProps) => {
    ...
    const resetTransaction = async () => {
      cashDetailStore.resetTransactionSelection();
    ...
    const handlePageChange = async (pageNumber: number) => {
      const transactionHistory =
        (cashDetailStore.transactionHistory &&
          cashDetailStore.transactionHistory.slice()) ||
        [];
      ...
      if (pageNumber == count) {
        const startDate = cashDetailStore.transactionSelection.startDate
          ? dayjs(cashDetailStore.transactionSelection.startDate)
              .startOf('day')
              .format()
          : null;
        const endDate = cashDetailStore.transactionSelection.endDate
          ? dayjs(cashDetailStore.transactionSelection.endDate)
              .endOf('day')
              .format()
          : null;
    ...
    const renderSingleTransactionIncon = (record: TransactionItem) => {
      if (
        Array<any>(
        ...
        if (
          cashDetailStore.cashDetail?.id === record.destinationAssetId &&
          ...
          return (
            <Box display="flex" alignItems="center" justifyContent={'center'}>
              ...
              &nbsp; {content.transactionHistory.transfer}
            </Box>
          );
        }
      } else if (
        ...
        if (
          cashDetailStore.cashDetail?.id === record.referentialAssetId &&
        ...
        Array<any>(TransactionTypeName.MoveToFund).includes(
    ...
    const renderFromToAssetType = (record: TransactionItem) => {
      if (
        ...
        if (
          record.destinationAssetId === cashDetailStore.cashDetail?.id &&
          record.destinationAssetType === AssetTypeName.cash
        ) {
          return (
            AssetTypeConstants[language][
              record.referentialAssetType || AssetTypeName.outside
            ] || ''
          );
        } else {
          return (
            AssetTypeConstants[language][
              record.destinationAssetType || AssetTypeName.outside
            ] || ''
          );
        }
      } else if (
          ...
          >
            ...
            <Box>
              <Table>
                ...
                <TableBody>
                  {transactionHistoryData
                    .slice(
                      (cashDetailStore.currentPage - 1) *
                        TransactionHistoryContants.itemsPerPage,
                      cashDetailStore.currentPage *
                        TransactionHistoryContants.itemsPerPage,
                    )
                    .map((record, i) => {
                      return (
                        ...
                        >
                          ...
                          <TableBodyCellSymbol align="right">
                            {renderSingleTransactionIncon(record)}
                          </TableBodyCellSymbol>
                          <TableBodyCellSymbol align="center">
              ...
              <Pagination
                ...
                currentPage={cashDetailStore.currentPage}
