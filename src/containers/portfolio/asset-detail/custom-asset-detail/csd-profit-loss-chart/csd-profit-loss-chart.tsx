import {
  ...
  import CSDLineChart from './csd-line-chart';
  
  const CSDProfitLossChart = observer(() => {
    const theme = useTheme();
    const isMobile = useMediaQuery(theme.breakpoints.down('sm'));
    const handleSelectedTypeChange = async (value: any) => {
      customAssetsDetailStore.fetchCustomAssetProfitLoss(value);
    };
      ...
      <>
        <Suspense fallback={<HypnosisLoading />}>
          ...
          >
            ...
            {customAssetsDetailStore.profitLossList &&
            customAssetsDetailStore.profitLossList.length > 0 ? (
              <CSDLineChart data={customAssetsDetailStore.profitLossList} />
            ) : (
              <></>
            )}
          </Card>
